...
// Enhanced server mapping with constraint-safe values
...
// Enhanced batch insertion with duplicate prevention
async function enhancedBatchInsert(servers, originalScoring, batchSize = 30) {
  ...
  try {
        ...
        for (const [batchIndex, server] of batch.entries()) {
          try {
            ...
            if (result.rows.length > 0) {
              ...
              if (totalInserted % 25 === 0 || totalInserted % 10 === 0) {
    ...
    console.error('❌ Enhanced batch insertion failed:', error.message);
    throw error;
  } finally {
    await client.end();
...
async function main() {
  try {
    console.log('🚀 Starting ENHANCED batch MCP server insertion...\n');
    ...
    // Preview
    console.log('\n📋 Preview of first 5 servers to insert:');
    newServers.slice(0, 5).forEach((server, i) => {
    ...
    // Execute enhanced insertion
    const insertedServers = await enhancedBatchInsert(newServers, originalScoring);

    console.log('\n🎉 Enhanced batch insertion completed successfully!');
    console.log(`📊 Final database now contains ${insertedServers.length + 1} total servers (including Context7)`);
